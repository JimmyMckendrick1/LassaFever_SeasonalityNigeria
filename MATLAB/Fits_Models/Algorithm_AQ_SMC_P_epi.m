...
% save_file: Full path for save file. This function will save at each generation,
% since if a time limit is set and reached everything is lost (Char)
...
if nargin == 0
    K = 2.0; Quantile = 0.5; T=2;
    M = 200; MaxIter = inf;    model = @Model_SEAIR_const;
    sigma=1;  beta = [1]; nu = [0.1]; eps = [1]; p = 0.8; gamma = [0.2]; B = [1/(53.5*365.0)]; mu  = [0.016];
    muI = [0.02]; N = [1000]; S0 = [950]; E0 = [2]; I0 = [1]; A0 = [1]; C0  = [1]; D0 = [0]; MaxTime = [80]; 
...
Num_iterations = zeros(1,K*M); % Number of parameter samples drawn in a generation
...
end % Every results table for that model is identicle
True_max_pdim = sum(p_set_dim(1,2:end),2); % the number of params once we don't segregate by name
saved_results = NaN(K*M,True_max_pdim+1); % Array for sampled parameters of all models, 
...
parfor i_particle=1:K*M
    ...
    if ~isreal(err)
        err = inf; % For some reason at some point negative params got 
        % through although invalid for their distribution, thus giving
        % complex params sometimes
    end
...
Total_iterations(1) = K*M;
% Set the next tolerance entry by the quantile of errors in the generation
% current generation
tolVec(g) = saved_results(M,end);
tolVec(2) = saved_results(floor(Quantile*M),end);

save_file_g = strcat(sprintf(save_name, g),'.mat');
save(save_file_g,'All_gen_results_cell','Total_iterations','tolVec','K','Quantile','T',...
    'seed','M','Priors_given_m','Input','PKernel','data','error_method','model','MaxIter','weights')

disp(g)
disp(toc)
g = 2;
while tolVec(g-1)/tolVec(g)-1> 0.005 && g <=T
    ...
    [~,idy] = sort(saved_results(:,end));
    saved_results = saved_results(idy,:);
    tolVec(g+1) = saved_results(floor(Quantile*M),end);
...
if g < T+1
    tolVec = tolVec(1:g);
    All_gen_results_cell = All_gen_results_cell(1:g-1);
    Total_iterations = Total_iterations(1:g-1);
    weights = weights(1:g-1);
    save_file_g = strcat(sprintf(save_name, g-1),'.mat');
    save(save_file_g,'All_gen_results_cell','Total_iterations','tolVec','PKernel','weights')
end
end